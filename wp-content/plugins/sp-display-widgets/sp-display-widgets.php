<?php
/*
Plugin Name: SP Display Widgets 
Text Domain: sp-display-widgets
Domain Path: /languages
Author: spoot1986
Author URI: https://cms3.ru/
Plugin URI: https://cms3.ru/kak-skryt-vidzhet-wordpress/
Description: Adds checkboxes to each widget to show or hide on site pages.
Version: 1.0.0
*/

global $sp_plugin;
$sp_plugin = new SPplugin();

class SPplugin{
    var $transient_name = 'sp_details';
    var $checked = array();
    var $id_base = '';
    var $number = '';
    ...
    function __construct(){
        ...
        $hook = apply_filters('sp_callback_trigger', 'wp_loaded');
        ...
        add_action('wp_ajax_sp_show_widget', array(&$this, 'show_widget_options'));
    ...
    function show_widget($instance) {
        ...
        $instance['sp_include'] = isset($instance['sp_include']) ? $instance['sp_include'] : 0;
        $instance['sp_logged'] = self::show_logged($instance);
        
        if ( ( $instance['sp_include'] && false == $show ) || ( 0 == $instance['sp_include'] && $show ) ) {
            return false;
        } else {
            $user_ID = is_user_logged_in();
            if ( ( 'out' == $instance['sp_logged'] && $user_ID ) ||
                ( 'in' == $instance['sp_logged'] && !$user_ID ) ) {
                return false;
            }
...
?>
<div class="sp_opts">
    <input type="hidden" name="<?php echo $widget->get_field_name('sp_include'); ?>" id="<?php echo $widget->get_field_id('sp_include'); ?>" value="<?php echo $instance['sp_include'] ?>" />  
    <input type="hidden" id="<?php echo $widget->get_field_id('sp_logged'); ?>" name="<?php echo $widget->get_field_name('sp_logged'); ?>" value="<?php echo $instance['sp_logged'] ?>" />
...
?>   
    <p>
        <label for="<?php echo $widget->get_field_id('sp_include'); ?>"><?php _e('Show Widget for:', 'sp-display-widgets') ?></label>
        <select name="<?php echo $widget->get_field_name('sp_logged'); ?>" id="<?php echo $widget->get_field_id('sp_logged'); ?>" class="widefat">
            <option value=""><?php _e('Everyone', 'sp-display-widgets') ?></option>
            <option value="out" <?php echo selected( $instance['sp_logged'], 'out' ) ?>><?php _e('Logged-out users', 'sp-display-widgets') ?></option>
            <option value="in" <?php echo selected( $instance['sp_logged'], 'in' ) ?>><?php _e('Logged-in users', 'sp-display-widgets') ?></option>
        </select>
    </p>

     <p>
    	<select name="<?php echo $widget->get_field_name('sp_include'); ?>" id="<?php echo $widget->get_field_id('sp_include'); ?>" class="widefat">
            <option value="0"><?php _e('Hide on checked pages', 'sp-display-widgets') ?></option> 
            <option value="1" <?php echo selected( $instance['sp_include'], 1 ) ?>><?php _e('Show on checked pages', 'sp-display-widgets') ?></option>
        </select>
    </p>    

<div style="height:150px; overflow:auto; border:1px solid #dfdfdf; padding:5px; margin-bottom:5px;">
    <h4 class="sp_toggle" style="cursor:pointer;margin-top:0;"><?php _e('Miscellaneous', 'sp-display-widgets') ?> +/-</h4>
    <div class="sp_collapse">
    ...
    ?>
        ...
        <label for="<?php echo $widget->get_field_id('page-'. $key); ?>"><?php echo $label .' '. __('Page', 'sp-display-widgets') ?></label></p>
    <?php } ?>
    </div>
    
    <h4 class="sp_toggle" style="cursor:pointer;"><?php _e('Pages') ?> +/-</h4>
    <div class="sp_collapse">
    ...
    <h4 class="sp_toggle" style="cursor:pointer;"><?php _e('Custom Post Types', 'sp-display-widgets') ?> +/-</h4>
    <div class="sp_collapse">
    ...
    <h4 class="sp_toggle" style="cursor:pointer;"><?php _e('Custom Post Type Archives', 'sp-display-widgets') ?> +/-</h4>
    <div class="sp_collapse">
    <?php foreach ( $this->cposts as $post_key => $custom_post ) {
    ...
    ?>
        ...
        <label for="<?php echo $widget->get_field_id('type-'. $post_key .'-archive'); ?>"><?php echo stripslashes($custom_post->labels->name) ?> <?php _e('Archive', 'sp-display-widgets') ?></label></p>
    <?php } ?>
    </div>
    <?php } ?>
    
    <h4 class="sp_toggle" style="cursor:pointer;"><?php _e('Categories') ?> +/-</h4>
    <div class="sp_collapse">
    ...
    <h4 class="sp_toggle" style="cursor:pointer;"><?php _e('Taxonomies', 'sp-display-widgets') ?> +/-</h4>
    <div class="sp_collapse">
    ...
    <h4 class="sp_toggle" style="cursor:pointer;"><?php _e('Languages', 'sp-display-widgets') ?> +/-</h4>
    <div class="sp_collapse">
    ...
    <p><label for="<?php echo $widget->get_field_id('other_ids'); ?>"><?php _e('Comma Separated list of IDs of posts not listed above', 'sp-display-widgets') ?>:</label>
...
<?php
    ...
    function update_widget_options($instance, $new_instance, $old_instance) {
        ...
        $instance['sp_include'] = ( isset($new_instance['sp_include']) && $new_instance['sp_include'] ) ? 1 : 0;
        $instance['sp_logged'] = ( isset($new_instance['sp_logged']) && $new_instance['sp_logged'] ) ? $new_instance['sp_logged'] : '';
...
$('.widgets-holder-wrap').on('click', '.sp_toggle', sp_toggle);
});
jQuery(document.body).bind('click.widgets-toggle', sp_show_opts);
function sp_show_opts(e){
    var target = jQuery(e.target);
    var widget = target.closest('div.widget');
	var inside = widget.children('.widget-inside');
	var opts = inside.find('.sp_opts');
	if(opts.length == 0){
	    return;
	}
...
function sp_toggle(){jQuery(this).next('.sp_collapse').toggle();}
/*]]>*/
</script>
<?php
    }
    ...
    function show_logged($instance) {
        if ( isset($instance['sp_logged']) ) {
            return $instance['sp_logged'];
        }
        
        if ( isset($instance['sp_logout']) && $instance['sp_logout'] ) {
            $instance['sp_logged'] = 'out';
        } else if ( isset($instance['sp_login']) && $instance['sp_login'] ) {
            $instance['sp_logged'] = 'in';
        } else {
            $instance['sp_logged'] = '';
        }
    ...
    function page_types(){
        $page_types = array(
            'front'     => __('Front', 'sp-display-widgets'), 
            'home'      => __('Blog', 'sp-display-widgets'),
            'archive'   => __('Archives'),
    ...
    function load_lang(){
        load_plugin_textdomain( 'sp-display-widgets', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
    }
